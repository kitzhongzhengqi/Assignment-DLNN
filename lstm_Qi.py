...
def forward(inputs, targets, memory):
    ...
    for t in range(input_length):
        ...
        # i = sigmoid(Wi * z + bi)
        i_s[t] = sigmoid(np.dot(Wi, zs[t]) + bi)
        ...
        # c_t = f * c_(t-1) + i * c_
        cs[t] = fs[t] * cs[t - 1] + i_s[t] * c_s[t]
        # output gate
        #o = sigmoid(Wo * z + bo)
        o_s[t] = sigmoid(np.dot(Wo, zs[t]) + bo)
        # new hidden state for the LSTM
        # h = o_gate * tanh(c_t)
        hs[t] = o_s[t] * np.tanh(cs[t])
        # DONE LSTM
    ...
    # activations = ()
    activations = (wes, cs, xs, zs, hs, fs, i_s, c_s, o_s, os, ps, ls)
...
def backward(activations, clipping=True):
    wes, cs, xs, zs, hs, fs, i_s, c_s, o_s, os, ps, ls = activations
    ...
    for t in reversed(range(input_length)):
        ...
        dhnext = np.dot(Why.T, do) + dh
        dcnext = o_s[t] * dtanh(np.tanh(cs[t])) * dhnext + dc
        # the differential of o_gate
        do_s = np.tanh(cs[t]) * dhnext
        # the differential of o_gate before activation
        do_s_pre_activation = dsigmoid(o_s[t]) * do_s
        # the weights and bias of o_gate
        dWo += np.dot(do_s_pre_activation, zs[t].T)
        dbo += np.sum(do_s_pre_activation,axis=-1, keepdims=True)
        # the differential of c_new
        dc_s = i_s[t] * dcnext
        dc_s_pre_activation = dtanh(c_s[t]) * dc_s
        dWc += np.dot(dc_s_pre_activation, zs[t].T)
        dbc += np.sum(dc_s_pre_activation,axis=-1, keepdims=True)
        # the differential of i_gate
        di_s = c_s[t] * dcnext
        di_s_pre_activation = dsigmoid(i_s[t]) * di_s
        dWi += np.dot(di_s_pre_activation, zs[t].T)
        dbi += np.sum(di_s_pre_activation,axis=-1, keepdims=True)
        # the differential of f_gate
        dfs = cs[t - 1] * dcnext
        dfs_pre_activation = dsigmoid(fs[t]) * dfs
        dWf += np.dot(dfs_pre_activation, zs[t].T)
        dbf += np.sum(dfs_pre_activation,axis=-1, keepdims=True)
        dc = fs[t] * dcnext
        #  the differential of z
        dz = np.dot(Wf.T, dfs_pre_activation) + np.dot(Wi.T, di_s_pre_activation) \
             + np.dot(Wc.T, dc_s_pre_activation) + np.dot(Wo.T, do_s_pre_activation)
        # cut the input vector
        dx = dz[hidden_size:]
        dh = dz[:hidden_size]
        # the differential of embedding layer
...
def sample(memory, seed_ix, n):
  ...
  """
    ...
    for t in range(n):
        ...
        c_s = np.tanh(np.dot(Wc, z) + bc)
        c = np.multiply(f, c) + np.multiply(i, c_s)
        o_s = sigmoid(np.dot(Wo, z) + bo)
        h = np.multiply(o_s, np.tanh(c))
        o = np.dot(Why, h) + by
        p = softmax(o)
...
if option == 'train':
    ...
    # prepare inputs (we're sweeping from left to right in steps seq_length long)
        ...
        if n % 1000 == 0:
            ...
            sample_ix = sample((hprev[:, :1], cprev[:, :1]), inputs[0][0], 2000)
